...
# count one tick down in the display timer
scoreboard players remove $match_prep_timer value 1

# if timer reaches end, run end function
execute if score $match_prep_timer value matches ..0 run return run function hipochallenge:sidebar_displays/match_request/end

# calculate remaining ticks as seconds for display
scoreboard players set $ticks_per_second local 20
scoreboard players operation $timer_seconds local = $match_prep_timer value
