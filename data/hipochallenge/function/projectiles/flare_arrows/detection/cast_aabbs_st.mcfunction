...
$execute at @s run function hipochallenge:std/aabb/cast_from_consts {selector:"@a[nbt=!{UUID:$(local_owner_uuid)},team=$(local_enemy_team),tag=!dead]",dx_const:"RECON_K2_FLARE_ARROW_DETECTION_DX",dy_const:"RECON_K2_FLARE_ARROW_DETECTION_DY",dz_const:"RECON_K2_FLARE_ARROW_DETECTION_DZ",cmd:"scoreboard players set $must_explode local 1"}
