#>hipochallenge:class/select/recon
#
# Selects the recon class.
#
# @context player
...
execute store result score @s class \
    run \
    data get storage \
    minecraft:hipochallenge consts.classes[{internal_name:"recon"}].id
...
data modify storage hc:temp msg_params.text.with[1] \
    set from storage \
    minecraft:hipochallenge consts.classes[{internal_name:"recon"}].name
