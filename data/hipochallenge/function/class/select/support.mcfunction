#>hipochallenge:class/select/support
#
# Selects the support class.
#
# @context player
...
execute store result score @s hc.Class \
    run \
    data get storage \
    minecraft:hipochallenge consts.classes[{internal_name:"support"}].id
...
data modify storage hc:temp msg_params.text.with[1] \
    set from storage \
    minecraft:hipochallenge consts.classes[{internal_name:"support"}].name
