...
# replace items in detected slot (must be wrapped in a function because macro
# could give an error if local_item_slot = -106)
...
# if item with custom data is in the player's cursor, replace cursor item
