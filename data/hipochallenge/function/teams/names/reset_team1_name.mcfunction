...
function hipochallenge:msg/all/inject_text/msg_all_inject {txt:'{"translate":"hc.msg.all.team1_changed_name_to","fallback":"The first team is now called %s","with":[{"translate":"hc.teams.team1.default_name","fallback":"%s","with":[{"nbt":"team1_name","storage":"minecraft:hipochallenge"}],"color":"green","underlined":true}]}'}
