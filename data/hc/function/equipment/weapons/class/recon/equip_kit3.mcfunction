# TODO: Add names and descriptions

# wind charge
item replace entity @s weapon.offhand with wind_charge

# sword
item replace entity @s hotbar.0 with paper
item modify entity @s hotbar.0 hc:weapons/class/recon/kit3/sword

# portal
item replace entity @s hotbar.1 with paper
item modify entity @s hotbar.1 hc:weapons/class/recon/kit3/portal

# deception effigy
item replace entity @s hotbar.2 with paper
item modify entity @s hotbar.2 hc:weapons/class/recon/kit3/deception_effigy