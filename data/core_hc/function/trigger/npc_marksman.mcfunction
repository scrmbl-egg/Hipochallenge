#>core_hc:trigger/npc_marksman
#
# Trigger for spawning a dummy with the marksman class attributes.
#
# @context player
...
execute at @s \
    ...
    summon minecraft:mannequin ~ ~ ~ { \
        ...
        description:{ \
            ...
            fallback:"%2$s %1$s", \
            with:[ \
                {translate:"hc.class.marksman.name"}, \
                {translate:"hc.class.marksman.icon"}, \
            ], \
        }, \
    }
...
data modify entity @n[tag=__hc.NewNpc] description.with[1].color \
    ...
    classes[{internal_name:"marksman"}].list_info.class_color
...
# reset and re-enable
scoreboard players reset @a npc_marksman
scoreboard players enable @a npc_marksman
