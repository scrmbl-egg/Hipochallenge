...
execute at @s \
    ...
    summon minecraft:mannequin ~ ~ ~ { \
        ...
        description:{ \
            ...
            fallback:"%2$s %1$s", \
            with:[ \
                {translate:"hc.class.assassin.name"}, \
                {translate:"hc.class.assassin.icon"}, \
            ], \
        }, \
    }
...
# reset and re-enable
scoreboard players reset @a npc_assassin
scoreboard players enable @a npc_assassin
