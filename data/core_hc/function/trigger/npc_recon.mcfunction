...
execute at @s \
    ...
    summon minecraft:mannequin ~ ~ ~ { \
        ...
        description:{ \
            ...
            fallback:"%2$s %1$s", \
            with:[ \
                {translate:"hc.class.recon.name"}, \
                {translate:"hc.class.recon.icon"}, \
            ], \
        }, \
    }
...
data modify entity @n[tag=__hc.NewNpc] description.with[1].color \
    ...
    classes[{internal_name:"recon"}].list_info.class_color
...
# reset and re-enable
scoreboard players reset @a npc_recon
scoreboard players enable @a npc_recon
