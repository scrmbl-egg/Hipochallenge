...
execute if predicate hc:team/is_in_match_pvp_team \
    run \
    data modify storage hc:temp select_perk.msg_params.text set value { \
...
execute if predicate hc:team/is_in_match_pvp_team \
    ...
    function core_hc:team/get_self_team { \
        out_storage:"hc:temp", \
        out_nbt:"select_perk.msg_params.team", \
    }

# get perk name
$data modify storage hc:temp select_perk.get_data_params set value { \
    class_id:0, \
    perk_id:$(value), \
    member_name:"name", \
    out_storage:"hc:temp", \
    out_nbt:"select_perk.msg_params.text.with[1]", \
}
execute store result storage hc:temp select_perk.get_data_params.class_id \
    int 1 \
    run \
    scoreboard players get @s hc.Class

function core_hc:perk/get_data_member \
    with storage hc:temp select_perk.get_data_params

# get kit name color by changing get_data_params
data modify storage hc:temp select_perk.get_data_params.member_name \
    set value "list_info.kit_color"
data modify storage hc:temp select_perk.get_data_params.out_nbt \
    set value "select_perk.msg_params.text.with[1].color"
...
execute unless predicate hc:team/is_in_match_pvp_team \
    run \
    data remove storage hc:temp select_perk.msg_params.text.with[0]

# send msg
execute if predicate hc:team/is_in_match_pvp_team \
    run \
    function hc:msg/team/send with storage hc:temp select_perk.msg_params
execute unless predicate hc:team/is_in_match_pvp_team \
    run \
    function hc:msg/private/send with storage hc:temp select_perk.msg_params
