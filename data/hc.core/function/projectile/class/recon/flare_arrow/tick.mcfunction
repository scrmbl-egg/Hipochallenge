#>hc.core:projectile/class/recon/flare_arrow/tick
#
...
execute unless data entity @s data."hc:entity/flare_arrow" \
    run \
    return run \
    function hc.core:projectile/class/recon/flare_arrow/add_data
...
execute if data storage hc:temp flare_arrow.explode \
    run \
    function hc.core:projectile/class/recon/flare_arrow/explode \
