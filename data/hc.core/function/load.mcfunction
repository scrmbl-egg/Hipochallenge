...
execute store result score hc:init_consts __hc.InitError \
    run \
    function hc.core:storage/init_consts
...
execute unless data storage hc:main vars \
    ...
    run \
    function hc.core:storage/init_vars
...
execute store result score hc:add_score_objectives __hc.InitError \
    run \
    function hc.core:score/add_all_objectives
...
execute store result score hc:init_triggers __hc.InitError \
    run \
    function hc.core:trigger/add_all
...
execute store result score hc:set_gamerules __hc.InitError \
    run \
    function hc.core:gamerule/set_all
...
schedule clear hc.core:ten_ticks
schedule function hc.core:ten_ticks 10t

schedule clear hc.core:second
